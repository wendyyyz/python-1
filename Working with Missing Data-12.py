...
for each in passenger_classes: 
    is_class = titanic_survival["pclass"] == each
    rows_by_class = titanic_survival[is_class]
    fares_by_class[each] = rows_by_class["fare"].mean()
...
drop_an_columns = titanic_survival.dropna(axis=1, how='any')
...
def null_count(series):
    col_null = pandas.isnull(series)
    null_elem = series[col_null]
    return len(null_elem)
